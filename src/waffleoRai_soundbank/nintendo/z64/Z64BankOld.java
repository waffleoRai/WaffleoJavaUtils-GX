...
class Z64BankOld {
	...
	public Z64BankOld(){this(0);}
	
	public Z64BankOld(int sfx_alloc){
	...
	public static Z64BankOld readBank(FileBuffer data){
		return readBank(data, -1, -1, -1);
	}
	
	public static Z64BankOld readBank(FileBuffer data, int i_count, int p_count, int e_count){
		//TODO Read ununsed envelopes too
		if(data == null) return null;
		//Determine alloc amount
		int e_alloc = (e_count>0)?e_count:4;
		Z64BankOld bank = new Z64BankOld(e_alloc);
	...
	public static Z64BankOld readUBNK(FileBuffer data) throws UnsupportedFileTypeException{
		if(data == null) return null;
		data.setEndian(true);
		//Read header block
		long mpos = data.findString(0, 0x4, UBNK_MAGIC);
		if(mpos != 0) throw new FileBuffer.UnsupportedFileTypeException("Z64BankOld.readUBNK -- Expecting UBNK magic number at offset 0!");
		...
		Z64BankOld bank = new Z64BankOld();
		...
		while(data.getCurrentPosition() < fsize){
			...
			if(cmagic.equals("META")){
				...
				if((ival & 0x1) != 0) bank.samples_by_uid = true;
		...
		//System.err.println("Z64BankOld.readUBNK || Linking...");
		for(int i = 0; i < 126; i++){
			...
			if(ib != null && ib.envelope == null && ib.snd_med == null){
				...
				if(ib.off_snd_lo != 0 && ib.off_snd_lo != -1){
				...
				if(ib.off_snd_med != 0 && ib.off_snd_med != -1){
				...
				if(ib.off_snd_hi != 0 && ib.off_snd_hi != -1){
	...
	public int readUWSD(FileBuffer data) throws UnsupportedFileTypeException{
		...
		if(mpos != 0) throw new FileBuffer.UnsupportedFileTypeException("Z64BankOld.readUWSD -- Expecting UWSD magic number at offset 0!");
		//Don't need to read the rest of the chunk til version update
		data.setCurrentPosition(0x10L);
		long fsize = data.getFileSize();
	...
	public void serializeTo(FileBuffer target){
		...
		int shamt = Z64BankOld.sermode_64?3:2;
		...
		for(int i = 0; i < itbl.length; i++){
			if(Z64BankOld.sermode_64) target.addToFile((long)itbl[i]);
		...
		for(int i = 0; i < icount; i++){
			...
			for(int j = i-1; j >= 0; j--){
				...
				if(block.data.instEquals(other.data)){
					//System.err.println("Z64BankOld DEBUG: Inst merge detected -- preset " + i + " to " + j);
	...
	private void mergeSamples(Z64BankOld other){
	...
	private void mergeInstruments(Z64BankOld other){
		if(other == null) return;
	...
	public void mergeElementReferences(Z64BankOld other){
		mergeSamples(other);
	...
	public SimpleBank toBank(FileBuffer sound_dat){
		...
		List<Z64BankOld> list1 = new ArrayList<Z64BankOld>(1);
		List<FileBuffer> list2 = new ArrayList<FileBuffer>(1);
		list1.add(this); list2.add(sound_dat);
		return toFontBank(list1, list2);
	}
	
	public static SimpleBank toFontBank(List<Z64BankOld> banks, List<FileBuffer> wavearc_dat){
		...
		ArrayList<Z64BankOld> processed_banks = new ArrayList<Z64BankOld>(bnk_count);
		for(Z64BankOld bank : banks){
			//First, clear names.
		...
		for(Z64BankOld bank : banks){
			...
			if(warc_idx < 0 || warc_idx >= war_count){
				System.err.println("Z64BankOld.toFontBank || --WARNING-- Bank " + b + " wave archive index " + warc_idx + " is invalid!");
				b++; continue;
			}
			...
			if(warc_data == null){
				...
				System.err.println("Z64BankOld.toFontBank || --WARNING-- Bank " + b + " points to empty wave archive: " + warc_idx);
				b++; continue;
			}
			...
			for(Z64BankOld prev : processed_banks){
			...
			if(bidx < 0) {
				System.err.println("Z64BankOld.toFontBank || --ERROR-- Bank " + b + " could not be added for some reason");
				return null;
			}
	...
	public static class NUSBank2SF2Converter extends ConverterAdapter{
		...
		public void writeAsTargetFormat(FileNode node, String outpath)
			...
			Z64BankOld bank = Z64BankOld.readBank(bankdat, i0, i1, i2);
	...
	public static void main(String[] args){
		...
		try{
			...
			Z64BankOld mybank = Z64BankOld.readBank(dat, icount, pcount, xcount);
