...
import waffleoRai_Containers.nintendo.NDKCtrFile;
...
public class CtrSeq {
	...
	public static final String MAGIC = "CSEQ";
	...
	public static CtrSeq readCSEQ(FileBuffer src) throws UnsupportedFileTypeException, IOException
	{
		if(src == null) return null;
		NDKCtrFile ctr_file = NDKCtrFile.readCtrFile(src);
		if(ctr_file == null) return null;
		if(!MAGIC.equals(ctr_file.getFileIdentifier())) throw new FileBuffer.UnsupportedFileTypeException("CtrSeq.readCSEQ || Source data does not begin with CSEQ magic number!");
		
		//DATA (Raw)
		FileBuffer data_sec = ctr_file.getSectionData(NDKSectionType.DATA);
		FileBuffer data = data_sec.createCopy(0xCL, data_sec.getFileSize());
		
		CtrSeq seq = new CtrSeq();
		seq.seqdata = new NinSeqDataSource(0, data, NinSeq.PLAYER_MODE_3DS);
		//seq.seqdata.freeSourceData();
		
		//LABL
		FileBuffer labl_sec = ctr_file.getSectionData(NDKSectionType.LABL);
		long cpos = 8L;
