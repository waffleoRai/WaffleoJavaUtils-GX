...
public class RevSeq {
	...
	public static final String MAGIC = "RSEQ";
	...
	public static RevSeq readRSEQ(FileBuffer src) throws UnsupportedFileTypeException, IOException
	{
		if(src == null) return null;
		NDKRevolutionFile rev_file = NDKRevolutionFile.readRevolutionFile(src);
		if(rev_file == null) return null;
		if(!MAGIC.equals(rev_file.getFileIdentifier())) throw new FileBuffer.UnsupportedFileTypeException("RevSeq.readRSEQ || Source data does not begin with RSEQ magic number!");
		...
		RevSeq seq = new RevSeq();
		seq.seqdata = new NinSeqDataSource(0, data, NinSeq.PLAYER_MODE_WII);
		//seq.seqdata.freeSourceData();
		
		//LABL
		FileBuffer labl_sec = rev_file.getSectionData(NDKSectionType.LABL);
		long cpos = 8L;
		...
		//DATA (Label split)
		//for(LabelSeq lseq : seq.subSeqs.values()) lseq.setData(seqdat, 0);
