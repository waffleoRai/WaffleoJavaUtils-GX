...
public class SMDSynthChannel extends DefaultSynthChannel{
	 ...
	 * @since 1.0.0
	 */
	public void smdNoteOn(long note_id, int note_rel, byte velocity) throws InterruptedException{
		int note = note_rel + (octave * 12);
		super.noteOn((byte)note, velocity);
		note_map.put(note_id, (byte)note);
		if(player != null)player.forwardNoteOnToListeners(index, (byte)note);
	}
	 ...
	 * @since 1.0.0
	 */
	public void smdNoteOff(long note_id){
