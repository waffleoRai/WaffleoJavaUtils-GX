...
public class NDKCtrFile extends NDKFile{

	private NDKCtrFile(FileBuffer src, int secCount)
	{
		data = src;
		sections = new NDKHeaderEntry[secCount];
	}
	
	public static NDKCtrFile readCtrFile(String path) throws IOException
	{
		FileBuffer file = FileBuffer.createBuffer(path, true);
		return readCtrFile(file);
	}
	
	public static NDKCtrFile readCtrFile(FileBuffer src)
	{
		...
		NDKCtrFile ctr = new NDKCtrFile(src, chunkCount);
		ctr.fileMagic = fileid;
		ctr.version_major = ver_major;
		ctr.version_minor = ver_minor;
		ctr.file_size = fileSize;
