...
//TS3 Script: Sims3.Gameplay.Visa.VisaManager
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		mVisitsToChina = stream.getFieldAsInt(PSID_MVISITSTOCHINA, true);
		mVisitsToEgypt = stream.getFieldAsInt(PSID_MVISITSTOEGYPT, true);
		mVisitsToFrance = stream.getFieldAsInt(PSID_MVISITSTOFRANCE, true);
		mDaysSpentInChina = stream.getFieldAsInt(PSID_MDAYSSPENTINCHINA, true);
		mDaysSpentInEgypt = stream.getFieldAsInt(PSID_MDAYSSPENTINEGYPT, true);
		mDaysSpentInFrance = stream.getFieldAsInt(PSID_MDAYSSPENTINFRANCE, true);
		mLastTimeOnVacation = stream.getFieldAsInt(PSID_MLASTTIMEONVACATION, true);
		
		visaCount = stream.getFieldAsInt(PSID_VISACOUNT, true);
		visaList.ensureCapacity(visaCount);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		...
		visaCount = visaList.size();
		ps.addInt(mVisitsToChina, PSID_MVISITSTOCHINA, true);
		ps.addInt(mVisitsToEgypt, PSID_MVISITSTOEGYPT, true);
		ps.addInt(mVisitsToFrance, PSID_MVISITSTOFRANCE, true);
		ps.addInt(mDaysSpentInChina, PSID_MDAYSSPENTINCHINA, true);
		ps.addInt(mDaysSpentInEgypt, PSID_MDAYSSPENTINEGYPT, true);
		ps.addInt(mDaysSpentInFrance, PSID_MDAYSSPENTINFRANCE, true);
		ps.addInt(mLastTimeOnVacation, PSID_MLASTTIMEONVACATION, true);
		ps.addInt(visaCount, PSID_VISACOUNT, true);
		for(VisaEntry visa : visaList){
