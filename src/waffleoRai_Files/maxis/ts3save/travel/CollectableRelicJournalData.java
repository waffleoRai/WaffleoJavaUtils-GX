...
//TS3 Script: Sims3.Gameplay.Skills.CollectableRelicJournalData
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		mGuid = stream.getFieldAsInt(PSID_MGUID, false);
		mNumFound = stream.getFieldAsInt(PSID_MNUMFOUND, true);
		mOldestAge = stream.getFieldAsInt(PSID_MOLDESTAGE, true);
		mHighestValue = stream.getFieldAsInt(PSID_MHIGHESTVALUE, true);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		if(ps == null) return;
		ps.addInt(mGuid, PSID_MGUID, false);
		ps.addInt(mNumFound, PSID_MNUMFOUND, true);
		ps.addInt(mOldestAge, PSID_MOLDESTAGE, true);
		ps.addInt(mHighestValue, PSID_MHIGHESTVALUE, true);
	}
