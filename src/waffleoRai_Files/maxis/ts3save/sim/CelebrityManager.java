...
//TS3 Script: Sims3.Gameplay.CelebritySystem.CelebrityManager
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		mLevel = stream.getFieldAsInt(PSID_MLEVEL, false);
		mPoints = stream.getFieldAsInt(PSID_MPOINTS, false);
		mBabyLevel = stream.getFieldAsInt(PSID_MBABYLEVEL, false);
		mFalselyAccusedCount = stream.getFieldAsInt(PSID_MFALSELYACCUSEDCOUNT, false);
		mPubliclyDisgracedCount = stream.getFieldAsInt(PSID_MPUBLICLYDISGRACEDCOUNT, false);
		mDiscountCount = stream.getFieldAsInt(PSID_MDISCOUNTCOUNT, false);
		mFreebiesCount = stream.getFieldAsInt(PSID_MFREEBIESCOUNT, false);
		mMoneySaved = stream.getFieldAsInt(PSID_MMONEYSAVED, false);
		mBarsVisitedCount = stream.getFieldAsInt(PSID_MBARSVISITEDCOUNT, false);
		mNumberOfTimesPhotographed = stream.getFieldAsInt(PSID_MNUMBEROFTIMESPHOTOGRAPHED, false);
		mAutographsSignedCount = stream.getFieldAsInt(PSID_MAUTOGRAPHSSIGNEDCOUNT, false);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		if(ps == null) return;
		ps.addInt(mLevel, PSID_MLEVEL, false);
		ps.addInt(mPoints, PSID_MPOINTS, false);
		ps.addInt(mBabyLevel, PSID_MBABYLEVEL, false);
		ps.addInt(mFalselyAccusedCount, PSID_MFALSELYACCUSEDCOUNT, false);
		ps.addInt(mPubliclyDisgracedCount, PSID_MPUBLICLYDISGRACEDCOUNT, false);
		ps.addInt(mDiscountCount, PSID_MDISCOUNTCOUNT, false);
		ps.addInt(mFreebiesCount, PSID_MFREEBIESCOUNT, false);
		ps.addInt(mMoneySaved, PSID_MMONEYSAVED, false);
		ps.addInt(mBarsVisitedCount, PSID_MBARSVISITEDCOUNT, false);
		ps.addInt(mNumberOfTimesPhotographed, PSID_MNUMBEROFTIMESPHOTOGRAPHED, false);
		ps.addInt(mAutographsSignedCount, PSID_MAUTOGRAPHSSIGNEDCOUNT, false);
	}
