...
package waffleoRai_Files.maxis.ts3save.sim.buffs;
...
import waffleoRai_Files.maxis.ts3enum.BuffNames;
import waffleoRai_Files.maxis.ts3enum.BuffOrigin;
import waffleoRai_Files.maxis.ts3enum.MoodAxis;
...
//TS3 Script: Sims3.Gameplay.ActorSystems.BuffInstance
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		mBuffGuid = stream.getFieldAsLong(PSID_MBUFFGUID, false);
		mEffectValue = stream.getFieldAsInt(PSID_MEFFECTVALUE, true);
		mAdjustedEffectValue = stream.getFieldAsInt(PSID_MADJUSTEDEFFECTVALUE, true);
		...
		mAxisEffected = stream.getFieldAsInt(PSID_MAXISEFFECTED, true);
		mBuffName = stream.getFieldAsString(PSID_MBUFFNAME);
		mDescription = stream.getFieldAsString(PSID_MDESCRIPTION);
		mBuffOrigin = stream.getFieldAsLong(PSID_MBUFFORIGIN, false);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		if(ps == null) return;
		ps.addLong(mBuffGuid, PSID_MBUFFGUID, false);
		ps.addInt(mEffectValue, PSID_MEFFECTVALUE, true);
		ps.addInt(mAdjustedEffectValue, PSID_MADJUSTEDEFFECTVALUE, true);
		...
		ps.addInt(mAxisEffected, PSID_MAXISEFFECTED, true);
		ps.addString(mBuffName, PSID_MBUFFNAME);
		ps.addString(mDescription, PSID_MDESCRIPTION);
		ps.addLong(mBuffOrigin, PSID_MBUFFORIGIN, false);
