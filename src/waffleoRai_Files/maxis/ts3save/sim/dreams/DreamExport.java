...
//TS3 Script: Sims3.Gameplay.DreamsAndPromises.DreamExport
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		mOwnerTreeId = stream.getFieldAsLong(PSID_MOWNERTREEID, false);
		mInstanceId = stream.getFieldAsInt(PSID_MINSTANCEID, false);
		mPrimitiveId = stream.getFieldAsInt(PSID_MPRIMITIVEID, false);
		mPromiseParentIndex = stream.getFieldAsInt(PSID_MPROMISEPARENTINDEX, true);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		...
		ps.addLong(mOwnerTreeId, PSID_MOWNERTREEID, false);
		ps.addInt(mInstanceId, PSID_MINSTANCEID, false);
		ps.addInt(mPrimitiveId, PSID_MPRIMITIVEID, false);
		ps.addInt(mPromiseParentIndex, PSID_MPROMISEPARENTINDEX, true);
