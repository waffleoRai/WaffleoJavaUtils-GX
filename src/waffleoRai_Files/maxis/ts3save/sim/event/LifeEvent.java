...
import waffleoRai_Files.maxis.ts3enum.LifeEventNature;
import waffleoRai_Files.maxis.ts3save.TS3Saveable;
import waffleoRai_Files.maxis.ts3save.sim.dreams.NodeSubject;
...
//TS3 Script: Sims3.Gameplay.ActorSystems.LifeEventManager.LifeEvent
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		mId = stream.getFieldAsInt(PSID_MID, false);
		timeSinceEvent = stream.getFieldAsFloat(PSID_TIMESINCEEVENT);
		mNature = stream.getFieldAsInt(PSID_MNATURE, true);
		...
		mSnapshot = stream.getFieldAsInt(PSID_MSNAPSHOT, true);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		...
		ps.addInt(mId, PSID_MID, false);
		ps.addFloat(timeSinceEvent, PSID_TIMESINCEEVENT);
		ps.addInt(mNature, PSID_MNATURE, true);
		...
		ps.addInt(mSnapshot, PSID_MSNAPSHOT, true);
	}
