...
//TS3 Script: Sims3.Gameplay.CAS.SimDescription
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		knownSims = stream.getFieldAsInt(PSID_KNOWNSIMS, true);
		friendSims = stream.getFieldAsInt(PSID_FRIENDSIMS, true);
		bestFriendSims = stream.getFieldAsInt(PSID_BESTFRIENDSIMS, true);
		jokesSuccesfullyTold = stream.getFieldAsInt(PSID_JOKESSUCCESFULLYTOLD, true);
		traitsLearned = stream.getFieldAsInt(PSID_TRAITSLEARNED, true);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		if(ps == null) return;
		ps.addInt(knownSims, PSID_KNOWNSIMS, true);
		ps.addInt(friendSims, PSID_FRIENDSIMS, true);
		ps.addInt(bestFriendSims, PSID_BESTFRIENDSIMS, true);
		ps.addInt(jokesSuccesfullyTold, PSID_JOKESSUCCESFULLYTOLD, true);
		ps.addInt(traitsLearned, PSID_TRAITSLEARNED, true);
