...
import waffleoRai_Files.maxis.ts3enum.Reputation;
...
//TS3 Script: Sims3.Gameplay.Socializing.ReputationStatistics
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		mReputation = stream.getFieldAsShort(PSID_MREPUTATION, true);
		mNumberOfRomanticBetrayals = stream.getFieldAsInt(PSID_MNUMBEROFROMANTICBETRAYALS, true);
		mLongestRomanceSimDescriptionId = stream.getFieldAsLong(PSID_MLONGESTROMANCESIMDESCRIPTIONID, false);
		mLongestRomanceSimDays = stream.getFieldAsFloat(PSID_MLONGESTROMANCESIMDAYS);
		mNumberOfCheatingRelationships = stream.getFieldAsInt(PSID_MNUMBEROFCHEATINGRELATIONSHIPS, true);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		if(ps == null) return;
		ps.addShort(mReputation, PSID_MREPUTATION, true);
		ps.addInt(mNumberOfRomanticBetrayals, PSID_MNUMBEROFROMANTICBETRAYALS, true);
		ps.addLong(mLongestRomanceSimDescriptionId, PSID_MLONGESTROMANCESIMDESCRIPTIONID, false);
		ps.addFloat(mLongestRomanceSimDays, PSID_MLONGESTROMANCESIMDAYS);
		ps.addInt(mNumberOfCheatingRelationships, PSID_MNUMBEROFCHEATINGRELATIONSHIPS, true);
	}
