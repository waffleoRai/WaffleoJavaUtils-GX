...
//TS3 Script:  Sims3.Gameplay.ActorSystems.Pregnancy
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		dadDescriptionId = stream.getFieldAsLong(PSID_DADDESCRIPTIONID, false);
		mHourOfPregnancy = stream.getFieldAsInt(PSID_MHOUROFPREGNANCY, true);
		mRandomGenSeed = stream.getFieldAsInt(PSID_MRANDOMGENSEED, true);
		mGender = stream.getFieldAsInt(PSID_MGENDER, false);
		mCurrentMoodScore = stream.getFieldAsInt(PSID_MCURRENTMOODSCORE, true);
		...
		mMomDeathType = stream.getFieldAsInt(PSID_MMOMDEATHTYPE, false);
		mDadDeathType = stream.getFieldAsInt(PSID_MDADDEATHTYPE, false);
		mForcedTrait = stream.getFieldAsLong(PSID_MFORCEDTRAIT, false);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		if(ps == null) return;
		ps.addLong(dadDescriptionId, PSID_DADDESCRIPTIONID, false);
		ps.addInt(mHourOfPregnancy, PSID_MHOUROFPREGNANCY, true);
		ps.addInt(mRandomGenSeed, PSID_MRANDOMGENSEED, true);
		ps.addInt(mGender, PSID_MGENDER, false);
		ps.addInt(mCurrentMoodScore, PSID_MCURRENTMOODSCORE, true);
		...
		ps.addInt(mMomDeathType, PSID_MMOMDEATHTYPE, false);
		ps.addInt(mDadDeathType, PSID_MDADDEATHTYPE, false);
		ps.addLong(mForcedTrait, PSID_MFORCEDTRAIT, false);
