...
import waffleoRai_Files.maxis.ts3enum.RomanceVisibilityFlags;
import waffleoRai_Files.maxis.ts3enum.RomanceVisibilityStateType;
...
//TS3 Script: Sims3.Gameplay.Socializing.RomanceVisibilityState
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		mSimAToSimB = stream.getFieldAsShort(PSID_MSIMATOSIMB, true);
		mSimBToSimA = stream.getFieldAsShort(PSID_MSIMBTOSIMA, true);
		mFlagField = stream.getFieldAsShort(PSID_MFLAGFIELD, true);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		if(ps == null) return;
		ps.addShort(mSimAToSimB, PSID_MSIMATOSIMB, true);
		ps.addShort(mSimBToSimA, PSID_MSIMBTOSIMA, true);
		ps.addShort(mFlagField, PSID_MFLAGFIELD, true);
