...
package waffleoRai_Files.maxis.ts3save.opportunities;
...
import waffleoRai_Files.maxis.ts3enum.OpportunityNames;
import waffleoRai_Files.maxis.ts3enum.WorldName;
import waffleoRai_Files.maxis.ts3save.TS3Saveable;
import waffleoRai_Files.maxis.ts3save.sim.event.EventListenerExportInfo;
...
//TS3 Script: Sims3.Gameplay.Opportunities.OpportunityHistory
	...
	public static class OppExportInfo extends TS3Saveable{
		...
		protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
			...
			index = stream.getFieldAsInt(PSID_INDEX, true);
			sourceSimDescriptionId = stream.getFieldAsLong(PSID_SOURCESIMDESCRIPTIONID, false);
			targetSimDescriptionId = stream.getFieldAsLong(PSID_TARGETSIMDESCRIPTIONID, false);
			targetInteractionNumberItemsRequired = stream.getFieldAsIntArray(PSID_TARGETINTERACTIONNUMBERITEMSREQUIRED, true);
			...
			chain = stream.getFieldAsLongArray(PSID_CHAIN, false);
			listenerCount = stream.getFieldAsInt(PSID_LISTENERCOUNT, true);
			listenerStates.ensureCapacity(listenerCount);
			for(int i = 0; i < listenerCount; i++){
			...
			localizationIndex = stream.getFieldAsInt(PSID_LOCALIZATIONINDEX, true);
			worldStartedIn = stream.getFieldAsLong(PSID_WORLDSTARTEDIN, false);
		...
		public void writeXMLNode(Writer out, String indent, String varName) throws IOException {
			...
			out.write(String.format(" %s=\"%s\"", XMLKEY_WORLDSTARTEDIN, WorldName.stringFromValue((int)worldStartedIn)));
		...
		public void addToPropertyStream(MaxisPropertyStream ps) {	
			...
			ps.addInt(index, PSID_INDEX, true);
			ps.addLong(sourceSimDescriptionId, PSID_SOURCESIMDESCRIPTIONID, false);
			ps.addLong(targetSimDescriptionId, PSID_TARGETSIMDESCRIPTIONID, false);
			if(targetInteractionNumberItemsRequired != null) {
				ps.addIntArray(targetInteractionNumberItemsRequired, PSID_TARGETINTERACTIONNUMBERITEMSREQUIRED, true);	
			}
			...
			if(chain != null) ps.addLongArray(chain, PSID_CHAIN, false);
			i = 0;
			for(EventListenerExportInfo eventListenerExportInfo : listenerStates){
				ps.addChildStream(eventListenerExportInfo.toPropertyStream(byte_order, verFieldSize), PSID_LISTENERSTATES+i);
				i++;
			}
			ps.addInt(listenerCount, PSID_LISTENERCOUNT, true);
			ps.addInt(localizationIndex, PSID_LOCALIZATIONINDEX, true);
			ps.addLong(worldStartedIn, PSID_WORLDSTARTEDIN, false);
	...
	protected boolean readPropertyStream_internal(MaxisPropertyStream stream) {
		...
		currentOpportunitiesCount = stream.getFieldAsInt(PSID_CURRENTOPPORTUNITIESCOUNT, true);
	...
	public void addToPropertyStream(MaxisPropertyStream ps) {	
		...
		ps.addInt(currentOpportunitiesCount, PSID_CURRENTOPPORTUNITIESCOUNT, true);
