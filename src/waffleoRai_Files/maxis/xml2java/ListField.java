...
public class ListField extends DataField{
	...
	public void writeVarSizeAddition(Writer out, String indent, String sizeVarName) throws IOException {
		...
		if(elementType.fieldType == TS3XJ.FIELD_TYPE_PRIM) {
			...
			out.write(useIndent + sizeVarName + " += ");
			if(eSize > 1) {
			...
			elementType.writeVarSizeAddition(out, useIndent + "\t", sizeVarName);
			out.write(useIndent + "}\n");
		}
	...
	public void writeReadBinCode(Writer out, String indent, String inputName) throws IOException{
		...
		out.write(useIndent + name + ".ensureCapacity(" + lenString + ");\n");
		out.write(useIndent + "for(int i = 0; i < " + lenString + "; i++){\n");
		out.write(useIndent + "\t" + javaTypeName + " " + elementType.name +  " = ");
		elementType.writeReadBinCodeList(out, inputName);
		out.write(";\n");
		out.write(useIndent + "\tif(" + elementType.name + " != null) ");
	...
	public void writeWriteBinCode(Writer out, String indent, String bufferName) throws IOException{
		...
		if(condition != null) {
			out.write(useIndent + "if(" + condition + "){\n");
			useIndent2 += "\t";
		}
		
		out.write(useIndent2 + "for(" + javaTypeName + " " + elementType.name + " : " + name + "){\n");
		out.write(useIndent2 + "\t");
		elementType.writeWriteBinCodeList(out, bufferName);
		out.write(";\n");
		out.write(useIndent2 + "}\n");
		
		if(condition != null) out.write(useIndent + "}\n");
		if(optional) {
			out.write(indent + "}\n");
		}
	}
	...
	public void writeWriteXmlCode(Writer out, String indent, String outputName) throws IOException{
		...
		out.write(useIndent2 + outputName + ".write(indent + \"\\t<List \");\n");
		out.write(useIndent2 + outputName + ".write(String.format(\" VarName=\\\"%s\\\">\\n\", " + xmlKeyName);
		out.write("));\n");
		out.write(useIndent2 + "for(" + javaTypeName + " " + elementType.name + " : " + name + "){\n");
		if(elementType.fieldType == TS3XJ.FIELD_TYPE_PRIM) {
			String fmt = DataField.getPrimWriteXmlStringFmt(elementType.typeName);
			out.write(useIndent2 + "\t" + outputName + ".write(String.format(");
		...
		else if(elementType.fieldType == TS3XJ.FIELD_TYPE_RESKEY) {
			out.write(useIndent2 + "\t" + elementType.name + ".writeXMLNode(" + outputName + ", indent + \"\\t\\t\"" + ");\n");
		}
		else {
			out.write(useIndent2 + "\t" + elementType.name + ".writeXMLNode(" + outputName + ", indent + \"\\t\\t\"" + ", null);\n");	
		}
		out.write(useIndent2 + "}\n");
		out.write(useIndent2 + outputName + ".write(indent + \"\\t</List>\\n\");\n");
